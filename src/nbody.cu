...
__global__ void MoveBodies(Particle* p_bodies, float* updated_masses,
		float p_deltaT) {
	int j = threadIdx.x;
...
__global__ void generateImage(Particle* bodies, char* imgData, int width,
	...
	int x_centre_pixel = (int) (((bodies[i].Position.X + fieldWidth)
			/ doubleFieldWidth) * img_width);
	int y_centre_pixel = (int) (((bodies[i].Position.Y + fieldHeight)
		...
		for (int iteration = 0; iteration < maxIteration; ++iteration) {
			...
			if (iteration % imageEveryIteration == 0) {
				...
				cudaMemcpy(d_imgData, imgData, imageSize,
						cudaMemcpyHostToDevice);
				generateImage<<<1, bodies.size()>>>(d_bodies, d_imgData,
